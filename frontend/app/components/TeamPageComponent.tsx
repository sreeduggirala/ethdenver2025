      ...
      {address ? (
        <>
          ...
          <div className="flex justify-between items-center mb-12 mt-4">
              ...
              <div className="text-green-400 text-2xl md:text-2xl font-mono">Bank: {balance}c</div>
              </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-16">
              ...
              kol.id ? (
                  ...
                  <NFTCard 
                      ...
                      points={kol.points}
                  />
                  <Link href={`/search?index=${index}`} className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-200 bg-black/60">
                      <button 
                  ...
                  onClick={() => handleEmptySlotClick(index)}
                  >
                  <NFTCard imageUrl="" username={`Slot ${index + 1}`} isEmpty={true} />
                  </Link>
              )
              ))}
          </div>
